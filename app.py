    ...
    def fetch_flight_data(bounding_box: Dict[str, float] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return pd.DataFrame(columns=['flight_id', 'latitude', 'longitude', 'altitude', 'ground_speed', 'heading', 'vertical_rate', 'timestamp'])
            ...
            # Process flights
            flights_data = []
            for idx, flight in enumerate(data['states'][:50]):  # Limit to 50 flights
                if flight[5] is not None and flight[6] is not None:
                    flights_data.append({
                        'flight_id': flight[1].strip() or f'Unknown-{idx}',
                        'latitude': flight[6],
                        'longitude': flight[5],
            ...
            df = pd.DataFrame(flights_data, columns=['flight_id', 'latitude', 'longitude', 'altitude', 'ground_speed', 'heading', 'vertical_rate', 'timestamp'])
...
def main():
    ...
    # Ensure real flight data exists with proper columns
    if 'real_flight_data' not in st.session_state or st.session_state['real_flight_data'].empty:
        st.session_state['real_flight_data'] = pd.DataFrame(
            columns=['flight_id', 'latitude', 'longitude', 'altitude', 'ground_speed', 'heading', 'vertical_rate', 'timestamp']
        )
    ...
    if analysis_type == "Flight Trajectories":
        ...
        # Plot real flights (only if real_data is not empty)
        ...
        real_flights = real_data['flight_id'].unique() if not real_data.empty else []
        ...
        compared_real_flights = st.multiselect(
            "Select Real Flights", 
            real_flights, 
            default=real_flights[:3] if real_flights else []
        )
